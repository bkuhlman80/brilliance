...
3. A visible italicized subtitle line right under the title, with no blank line before it — and
   ...
   - **Category 3 (Household Vignettes):** no subtitle line at all — dates live at the beat level
     instead (see below).
   - **Category 5 (terminal reflection):** date only, no time — `*2031-05-25*`.
...
## The chapter spine — 19 stops, fixed order
...
**Ruling (Ch9): categories 2 and 3 swapped, permanently, chapter spine runs chronological.**
Category 3 (Household Vignettes) now sits before category 2 (Cheryl's Lunch) in every chapter.
Reason: the spine is meant to run in the story's own chronological order, and category 2's lunch
date consistently falls a month or more after category 3's vignette dates in the chapters checked
so far (e.g. Ch9: vignettes Mar 12–Apr 1, lunch Apr 30). Brian is applying this swap directly across
all chapters himself; this doc's spine order is updated to match and is the current source of truth
going forward. Nothing else about either category's internal rules (interweave, visible dates,
etc.) changes.
...
| 19   | 5        | Chapter-Terminal Coda                   | a. The Specimen(s) — b. The Point                            |
