...
| 3 | Genetic knob-3 | Eukaryote | Modeler | Foraging | Interiority | exploit ↔ explore | Habit mode ↔ Improv mode |
| 4 | Genetic knob-4 | Eumetazoa | Reviser | Home | Excitability | comfy ↔ active | Warm mode ↔ Cool mode |
...
| 7 | Neuronal knob-3 | Mammal | Modeler | Attention | Autonomy | immersive ↔ surveying | Deep mode ↔ Meander mode |
...
| 10 | Symbolic knob-2 | Settlement-Human | Regulator | Conscientious | Stability | High ↔ low | Tidy mode ↔ Messy mode |
...
* **Warm ↔ Cool** (Home) — settles in the warmest corner it can find vs. doesn't care about warmth, parks where the people are.
...
* **Deep ↔ Meander** (Attention) — all of it close and loud, nothing else in the room, vs. the whole field at once with nothing pulling harder than anything else.
...
* **Tidy ↔ Messy** (Conscientious) — the schedule is load-bearing vs. the schedule is a suggestion.
...
- **Ch 5 — Effector / Credit (short-horizon ↔ long-horizon).** `OBJECT` shows while awake and is dark during sleep — the unit does not "dream" at this cell. Wake/sleep timing runs on the emergent allostatic process installed at Eumetazoa; the Credit dial has no bearing on it.
- **Ch 6 — Regulator / Coping (proactive ↔ reactive).** *Reactive* (suppressive) → falls through to Ch 5: `OBJECT` during waking, dark during sleep. *Proactive* (expressive) → `VERB + OBJECT` always, including during sleep. A sleeping unit in proactive might display `IGNORE WORLD` or `CHARGE UNIT`.
...
The Effector knob is special — it sits below the stack the higher knobs model. It is what's modeled, not a modeler. Wake and sleep run on the emergent allostatic process installed at Eumetazoa, independent of the knob; Credit shapes which percept wins the attention race once the unit is awake. Together they are the substrate the higher layers read or estimate; the Effector knob doesn't add a display layer of its own beyond the bottom `OBJECT` band.
