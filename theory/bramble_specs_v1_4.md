...
| Introductory chapter | Line-knob slot | Cell | Slot | Tendency | Motivation | Lab (derived ↔ primordial) | Mira (derived ↔ primordial) |
| -------------------- | --------------- | ---------------- | --------- | ------------------ | -------------- | ------------------------- | -------------------------- |
| 1 | Genetic knob-1 | Protocell | Effector | Going | Persistence | cranked ↔ unwound | Live ↔ Dead |
| 2 | Genetic knob-2 | Prokaryote | Regulator | Effort | Coordination | aerobic ↔ anaerobic | Sloth ↔ Sprint |
| 3 | Genetic knob-3 | Eukaryote | Modeler | Exploiting | Interiority | exploit ↔ explore | March ↔ Dance |
| 4 | Genetic knob-4 | Eumetazoa | Reviser | Action | Excitability | active ↔ comfy | Busy ↔ Warm |
| 5 | Neuronal knob-1 | Bilaterian | Effector | Orienting | Impetus | Impulsive ↔ Inhibited | Improvise ↔ Think |
| 6 | Neuronal knob-2 | Vertebrate | Regulator | Coping | Mastery | reactive ↔ proactive | Little ↔ Beast |
| 7 | Neuronal knob-3 | Mammal | Modeler | Attention | Autonomy | surveying ↔ immersive | Wide ↔ Deep |
| 8 | Neuronal knob-4 | Primate | Reviser | Agreeable | Affiliation | agreeable ↔ selfish | Respect ↔ Savage |
| 9 | Symbolic knob-1 | Band-Human | Effector | Resolve | Commitment | effective ↔ neurotic | Clutch ↔ Stall |
| 10 | Symbolic knob-2 | Settlement-Human | Regulator | Conscientious | Stability | orderly ↔ kludgy | Preppy ↔ Messy |
| 11 | Symbolic knob-3 | City-Human | Modeler | Extraverted | Representation | assertive ↔ passive | Boss ↔ Vibe |
| 12 | Symbolic knob-4 | Empire-Human | Reviser | Honest | Universality | candid ↔ canned | Real ↔ Flex |
...
Column convention: the Lab and Mira columns read derived pole first, primordial pole second. The derived pole is the knob's own clade primary, the newer installed layer; the primordial pole is the prior clade's primary, the ancestor that shows through when the knob is at its low setting. Derived and primordial are properties of knob settings, not a ranking of the four functions, which remain equal primaries.
...
* **Sloth ↔ Sprint** (Effort) — slow endurance off the battery vs. a supercapacitor burst that then sits and recovers. Names a knob the team only ever called aerobic/anaerobic, and one already shelved by the time the name arrives.
* **March ↔ Dance** (Exploiting) — the worn efficient groove vs. making the path up as it goes.
* **Busy ↔ Warm** (Action) — doesn't care about warmth, parks where the people are vs. settles in the warmest corner it can find. Busy is the derived setting, Warm the primordial.
* **Improvise ↔ Think** (Orienting) — moves on the freshest cue, acts before the pattern confirms vs. holds off on a single unfamiliar event, waits for it to repeat. Improvise is the derived setting (recency, eligibility trace); Think the primordial (novelty, latent inhibition). Bramble's setting: [OPEN, not yet decided; leave placeholder, do not invent].
* **Little ↔ Beast** (Coping) — hangs back and waits to be sure vs. leans hard at anything that might pay. Little is the derived setting, Beast the primordial. Little is a size word, not a deficit; it thinks it's little.
* **Wide ↔ Deep** (Attention) — the whole field at once with nothing pulling harder than anything else vs. all of it close and loud, nothing else in the room. Wide is the derived setting, Deep the primordial.
* **Respect ↔ Savage** (Agreeable) — defers, won't impose, vs. out for its own. Respect is the derived setting, Savage the primordial.
* **Clutch ↔ Stall** (Resolve) — composed, enters freely, comes through vs. the engine cuts and it can't get moving. Clutch is the derived setting, Stall the primordial.
...
- **Suppressive setting** (proactive, immersive, self-favoring) → the new layer is dormant; the unit falls through to whatever the cell beneath is doing, including its on/off.
- **Expressive setting** (reactive, surveying, agreeable) → the new layer runs always-on, generating display content out of its own model whether or not the lower layer has anything live for it to read. The expressive setting therefore overrides the lower cell's sleep-dark.
...
- Eumetazoa (knob-4): `STARTLED` / `AWAKE` / `SLEEPING` — the arousal state. Shows only when knob-4 is at volume up (active); at volume down (comfy) the panel is off and the screen falls through to knob-3.

**Sleep and the three off-states.** Sleep renders as the word `SLEEPING` on the arousal panel, not as an unlit band. A comfy unit (knob-4 at volume down) that is asleep therefore does not read `SLEEPING` — its arousal panel is off; the screen falls through to knob-3 (blank on the tetrapod) and then knob-2, reading `STOP` if knob-2 is at volume up and blank otherwise. Three states stay distinct: `DARK` (a lit `OBJECT` band, the unit attending to darkness), `SLEEPING` (the arousal panel's sleep word), and blank (no active knob has content to show).
...
1. **`OBJECT` and `VERB + OBJECT`, when produced by direct read** (Ch 5 awake, Ch 6 reactive, or Ch 7 immersive-over-reactive). Honest, involuntary first-order output. Cannot lie. True to Bramble's knowledge base — the unit can sincerely orient toward a bowl that is not where it believes it is.
...
## Surveying estimates always; proactive-surveying is the sharpest case
...
In *reactive-surveying* the underlying behavior is agency-shaped, so the estimated `VERB + OBJECT` is usually close to true; the self-model narrates real agency. In *proactive-surveying* the underlying behavior was capture rather than agency, and the estimate dresses a reaction as agency — the surface output is identical to reactive-surveying, but the self-model has invented the action it claims to be naming. This is the illusion of conscious will, rendered on the face. The seam the book banks for later self-model distortion.

*Epistemic status: derived consequence of the modeling-not-reading mechanism plus the surveying-always-on rule. Both are ratified; the proactive-surveying confabulation follows.*
