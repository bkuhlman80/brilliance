...
def chapter_key(title: str):
    """Sort `Chapter 2` before `Chapter 10`, and anything unnumbered last."""
    m = re.match(r"Chapter (\d+)\b", title)
    return (0, int(m.group(1))) if m else (1, 0)


def collect_stats(md_path: str, opts) -> dict:
    """Re-derive a chapter's numbers from its source, exactly as it was built."""
    with open(md_path, encoding="utf-8") as fh:
        raw = fh.read()
    blocks = load(md_path, opts.dash, opts.brackets)
    stem = os.path.splitext(os.path.basename(md_path))[0]
    sections = split_sections(blocks, fallback_title=stem)
    if not opts.keep_byline:
...
def render_readme(rows: list[dict], wpm: float, orphans: list[str]) -> str:
    head = ["Ch", "Title", "Source", "H1s", "Words", "Lines"]
    head += [f"{s:g}x" for s in SPEEDS]
    align = ["---:", "---", "---", "---:", "---:", "---:"] + ["---:"] * len(SPEEDS)

    body = []
    for r in rows:
        body.append([
            r["number"] or "--", r["title"], r["source"], str(len(r["h1s"])),
            f"{r['words']:,}", f"{r['lines']:,}",
        ] + [listen_time(r["words"], wpm, s) for s in SPEEDS])

    total_words = sum(r["words"] for r in rows)
    total = [
        "", f"**{len(rows)} chapters**", "",
        str(sum(len(r["h1s"]) for r in rows)),
        f"**{total_words:,}**", f"{sum(r['lines'] for r in rows):,}",
    ] + [f"**{listen_time(total_words, wpm, s)}**" for s in SPEEDS]
    ...
    out += [row(cells) for cells in body]
    out.append(row(total))

    multi = [r for r in rows if len(r["h1s"]) > 1]
    if multi:
        out += ["", "## Files with more than one H1", ""]
        for r in multi:
            out.append(f"- **{r['source']}**")
            out += [f"  - {t}" for t in r["h1s"]]

    if orphans:
        out += ["", "## No source found", "",
                "Built from a source no longer in the chapters directory:", ""]
        out += [f"- {name}" for name in orphans]

    return "\n".join(out) + "\n"


def write_readme(out_dir: str, source_dir: str, opts) -> str | None:
    """Rebuild the README from every EPUB present, not just the one converted.

    Stats come from each book's source markdown, so a single-chapter run leaves
    the other rows correct rather than stale.
    """
    rows, orphans = [], []
    for epub in sorted(glob.glob(os.path.join(out_dir, "*.epub"))):
        stem = os.path.splitext(os.path.basename(epub))[0]
        src = os.path.join(source_dir, stem + ".md")
        if os.path.isfile(src):
            rows.append(collect_stats(src, opts))
        else:
            orphans.append(os.path.basename(epub))
    if not rows and not orphans:
        return None
    rows.sort(key=lambda r: chapter_key(f"Chapter {r['number']}" if r["number"] else ""))
    path = os.path.join(out_dir, "README.md")
    with open(path, "w", encoding="utf-8") as fh:
        fh.write(render_readme(rows, opts.wpm, orphans))
    return path
...
def main(argv=None) -> int:
    ...
    if not args.no_readme:
        for out_dir, source_dir in dict.fromkeys(written):
            path = write_readme(out_dir, source_dir, args)
            if path:
