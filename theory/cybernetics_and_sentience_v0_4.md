...
This doc grounds the Venn architecture. The line-by-line diagram, the label libraries, and the row-slot tables appear in each of the Inheritance_Lines line docs (the shared framework is duplicated across all three; per-cell tables for a given line are in that line's file).
### **The bramble and the trellis**

The framework's central metaphor is a pair. *Bramble* is the unit — the cybernetic organism whose architecture and personality the framework specifies, instantiated as the lab's developmental robot at each of twelve cells. *Trellis* is the framework structure on which the bramble grows — the architectural scaffold of cybernetic functions, motivations, signatures, capacities, and breakthroughs, organized line by line and cell by cell across the twelve. The metaphor lands in scene at Ch 1: Lisa closes her foundation talk by gesturing forward to "what runs on the Protocell as substrate" — regulation, modeling, revision, nervous tissue, brains, language — and accidentally lands *the rest is the trellis*. The team picks the phrase up, and from that point onward "the trellis" is the team's working vocabulary for the framework's architecture. The lab's public-facing name follows the same image: bramble grows on trellis.
...
DeYoung's Stability and Plasticity metatraits do not sit at the triple-pair axes. They sit at single-primary positions in the Empire-Human Venn, where the Big Five empirically lives. Stability sits at the Effector (Commitment at Empire-Human). Plasticity sits at the Modeler (Representation at Empire-Human). See `Inheritance_Lines_symbolic` doc for the Empire-Human Big Five placements.
...
The reading applies only to the tendencies built as graded dials, the ones with a continuous range from a low end to a high end. It does not apply to the toggles, which select between two modes of a single layer rather than adding a layer that can be removed. In the current roster the genetic-line tendencies are all toggles, as is the neuronal-line Chronotype, and these carry no dormant setting and no low-end fall-through.
...
### The Bridge-Cell Reflex Repertoire
At the last cell of each line, the Operator signature (Regulator × Reviser) is that line's reflex repertoire: the stored, automatic response patterns the next line will organize. The genetic line's last cell, Eumetazoa, carries the Reflex Repertoire, the coordinated excitable response patterns nervous tissue makes possible. The neuronal line's last cell, Primate, carries the Sociometer, the automatic social-evaluation patterns mentalization makes possible. The two sit in the same slot, one line apart, and play the same role: each is the raw behavioral material the next line is built from.

"Reflex" here means automatic relative to the space the next line builds, not stimulus-response in the spinal sense. Eumetazoan reflexes are automatic relative to the mind the neuronal line will build. The Primate's social reflexes, which track inclusion, exclusion, standing, and who allies with whom, are automatic relative to the culture the symbolic line will build. These social reflexes are not representation-free: they run on the generative model the neuronal line spent four cells assembling. What makes them reflexes is that they fire below deliberation, not that they lack a model.

The organizing work happens at a different level each time. The genetic-to-neuronal transition organizes parts within a body: the reflexes of nervous tissue become a brain. The neuronal-to-symbolic transition organizes bodies within a collective: the social reflexes become a superorganism. This extends the constituent-clade pattern already in the framework, in which Eumetazoa are built from eukaryotic cells and empires from cities, to the reflex repertoire at each line boundary. The reflexes are the material; the next line's architecture is the organization. At the symbolic line that architecture is the we-mode: Commitment binds the social reflexes into joint purpose, and symbols let it scale to non-kin who never meet.

Epistemic status: the bridge-cell parallel is a framework theoretical commitment, built on the existing Operator-signature and constituent-clade structure.
...
*Framework argument. Backs Lisa's Ch 5 framework beat, part 2 — the line boundary and the goo change. This is theory-side and ungated; the section "What surfaces in Ch 5" marks the slice that is allowed into the chapter. Epistemic-status tags appear inline: **[established]** for textbook formalism, **[framework]** for a well-supported framework interpretation, **[open]** for a call held deliberately loose.*

---

## What this is for

Each inheritance line works a different kind of stuff. The genetic line works one kind, the neuronal line another, the symbolic line a third. Lisa names this change at the genetic-to-neuronal boundary in Chapter 5, where she says the goo of inheritance changes when the Effector motivation changes. This document gives the full theory behind that one sentence.
...
Three patterns present themselves. The first two fail or under-deliver, which is what motivates the third.

**Monotonic reflexivity.** **[framework, true but thin]** The capacity for reflexive revision grows across the lines. Genes cannot be about genes — variation is blind. Neurons model lower layers but cannot operate on the modeling-relation itself — revision is bounded. Symbols can take their own operations as objects without limit — revision is unbounded, which is why the symbolic line accelerates and the others do not. This is true and it is in the framework already, but it only orders the lines by degree. It does not explain why there are three, why they must emerge in this order, or why the genetic and symbolic lines rhyme.
...
**The Venn of lines.** This is the structure that survives, argued in full below.
...
## What surfaces in Ch 5

At Chapter 5 the reader has only two lines in view, so Lisa's beat is gated to the two-line slice of the Venn. **[framework — gating discipline]**

The line boundary is the move from the Effector line to the Regulator line. The Effector swap that defines the boundary is, at this higher scale, the crossing from the function-of-doing to the function-of-regulating. That is why an Effector swap is a boundary and the other swaps are not.

The goo change Lisa names is the change from Boltzmann goo to Friston goo — from the entropy of matter, which the genetic line dissipates, to the entropy of an agent's states, which the neuronal line regulates by minimizing surprise. The word *agent* is load-bearing: the neuronal line is the first line that is an agent in the regulator's sense, a thing that maintains and updates itself, which is why a free-energy account fits it and a thermodynamic one fit the line before it.

The value change can be named without the full apparatus. The neuronal line is where the system starts paying to find out, not only to reach its setpoints — where epistemic value joins pragmatic value as a driver. Boldness is the name of that willingness.

Held out of Ch 5: the Modeler/symbolic line, the Reviser/fourth line, the Darwin center, and the word "Venn" applied to the lines. Those belong to later chapters or to the framework only. Lisa at Ch 5 gives the genetic-to-neuronal slice and no more.
...
**Framework interpretation, well supported.** The mapping of functions to goos (Effector–Boltzmann, Regulator–Friston, Modeler–Shannon); the Goo Venn and its derivation of the line order; Darwin as the central breakthrough; the four-line cap; epistemic value arriving at the neuronal line.

---

# Understanding vs. capability

**The stance.** **[framework]** ABHOP is built to show how minds evolved, not to build a capable mind. The distinction is understanding vs. capability, and it governs every design choice. A capable system does the task well. An instrument for understanding makes the path to the task legible. Bramble is the second kind. The build discipline that enforces this is owned by `bramble_specs`: the least machinery that renders each cell honestly, plus the clade-lock that withholds what a cell has not earned. This section states only what that discipline is for.
