...
The free energy principle gives a substrate-independent formulation: any system that maintains itself against entropy admits a description in which a quantity equivalent to free energy is at a minimum. That quantity decomposes as expected free energy into pragmatic value (preferred outcomes) and epistemic value (information gain). Safron & DeYoung (2021) maps this onto CB5T's metatraits: the Stability metatrait tracks pragmatic value (precision over preferences, the C matrix); the Plasticity metatrait tracks epistemic value (precision over policies).
...
- **Neuronal line ratchets Friston goo.** A system that persists occupies a narrow set of expected states. The surprise that would measure this directly is intractable, so the formalism uses an upper bound on it: variational free energy, in the form *complexity minus accuracy*. The bound falls two ways, by perception updating the model and by action changing the world to match the model. What the description tracks is a unit that stays out of improbable states, and selection preserves the units whose bound runs lower. Affective and motivational structure (sensorimotor reentrant signaling shaped into trait-level dispositional architecture, empirically documented from vertebrates up) is what running Friston goo looks like at the unit level. The Venn of Lines section below gives the full argument; this row records the line-level commitment.
...
**Friston free energy.** **[established as formalism; framework as a line-goo]** A system that persists occupies a narrow set of expected states, which is to say the long-run entropy of its own state distribution is low. The surprise that would measure this directly is intractable, so the formalism substitutes an upper bound on it: variational free energy, which takes the form *complexity minus accuracy*. The bound falls two ways, by perception updating the model and by action changing the world to match the model. Stated this way the principle is a redescription and not a cause, which is Friston's own reading of it, and it is why the quantity carries no explanatory load here. Selection does that work. Friston free energy is an information quantity (a divergence, Shannon-family) cast in the form of a thermodynamic potential (Helmholtz-family). Helmholtz free energy is `F = U − TS`, the energy available to do work at constant temperature; the name is his, and so is the older root of the idea, perception as unconscious inference. This is what the Regulator's work is counted in.
...
| Friston | sighted: the only one of the four with the representation relation inside the formula | bounded: defined over a system that occupies a narrow set of expected states |
...
| Friston and Kolmogorov | Operator, diagonal | none direct; the apparent link reaches Kolmogorov through Shannon and is two relations composed |
...
Expected free energy decomposes into two terms, and both are higher where expected free energy is lower. **[established]**
...
| Neuronal | Regulator | **Friston** | regulation by a model; departure from expected states |
...
**The refused bridges.** **[established]** Boltzmann linked Darwin to thermodynamics in an 1886 lecture, casting the struggle for existence as a struggle for available free energy. Lotka's 1922 maximum power principle set the template that repeats afterward: a careful scientist proposes that selection maximizes a thermodynamic quantity, the proposal turns out not to be derivable, and less careful people adopt it anyway. Schrödinger's 1944 lectures split down the middle, though not along the line usually reported. The aperiodic crystal paid off. Negative entropy drew doubt and opposition from physicist colleagues, and the note printed with the first edition answers them. He concedes that free energy would have been the better term had he been writing for physicists, and explains that he avoided it for his actual readers because it sits close enough to plain energy to hide the contrast he wanted drawn. He gave up the word for one audience and kept the claim. To F. Simon's sharper objection, that the account does not say why an organism needs ordered compounds rather than fuel, he answered by adding the heat the organism sheds, which extends the argument rather than withdrawing it. What circulates now is that he admitted a mistake and should have written free energy. **[framework]** The distance between the note and its reputation is the mechanism described here, running on a primary source rather than on a field.
...
The line runs through careers rather than between them. **[framework]** It splits Odum. It splits Schrödinger inside a single book. It splits Kauffman, whose fourth law is declined under "Why the Turn Does Not Return" in Part IV while his collectively autocatalytic sets are kept there as an accounting. So the pattern does not record which scientists were careless. It records which claims a field can absorb.
...
**Shape is not the whole of it. Route matters too.** **[framework]** Frank's Fisher-information result and England's self-replication work are both physics brought to selection, and their receptions could not be further apart. Part of that is shape: Frank restates what the Price equation already says and proposes nothing new about why anything happens, while England proposes a physical tendency toward better dissipation, which is a drive. The rest is route. Frank published inside evolutionary biology, in a journal its practitioners read, where the claim was evaluated by people equipped to check it against what they already had. England published in chemical physics, and the version that reached biologists arrived through popular science coverage that had already overstated it. A claim entering a field through its own literature gets read. A claim arriving as news gets classified. **[open]** Whether route or shape carries more weight here is not settled, and the two are correlated in every case listed above, because a proposal that a field's own journals will publish is usually one that field can already evaluate.
...
The framework's own neuronal-line formalism is held to that line. **[framework]** The Regulator's work is counted in variational free energy, and the entry under "The four measures that name a goo" records that the principle is a redescription rather than a cause, on its originator's reading. The framework claims no more for that quantity than the person who wrote it down does.
...
keeps Lane's version of it. Kauffman is declined for the fourth law only. Collectively
autocatalytic sets are a closure result about what a chemical network can sustain, which
is an accounting rather than a drive, and the framework keeps it. The history of these
proposals and their reception is under
...
gate against a stored reference, which is Friston.
