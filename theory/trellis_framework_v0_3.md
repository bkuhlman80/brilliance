...
*Working document. v0.1. Synthesis of three prior docs (the deep-time evolutionary substance pool, Bramble's fictional cognitive-architecture spec, and the foundational Venn / cybernetics doc) into one canonical Trellis-framework reference.*

This doc carries three movements.

- **Part I — The deep-time evolutionary story.** What personality is, what each inheritance line is made of, how the bramble grows up the trellis, the ecology across lines, the genealogy of the thread, and what ABHOP adds.
- **Part II — Bramble's cognitive rendering.** The cognitive architecture the framework specifies for Bramble: the Evolutionary Algorithm, the two minds, why there is only one mind at Eukaryote, the clade-locking principle, the macro/micro division of labor, the growth map across cells, consolidation and sleep, and the per-cell software constraints. Part II is Part I rendered as a working cognitive architecture; its components are real and citable, the synthesis is the framework's own.
...
**Authority for framework labels.** The per-cell label libraries — the specific motivation names, capacity names, signature constructs, and breakthroughs at each of the twelve cells — are not in this doc. They live in the **Inheritance_Lines docs** (one per line: genetic, neuronal, symbolic). When this doc references a motivation by name (Persistence, Excitability, Commitment, and so on) or a row-slot label, the authoritative definition is the corresponding line's Inheritance_Lines doc. If a name in this doc and a name in an Inheritance_Lines doc disagree, the Inheritance_Lines doc wins and this doc is the derivative to fix.
...
- **Neuronal** stores information in neural patterns, transmits via behavior and learning, modifies through experience. It ratchets Friston goo (the variational free energy an agent minimizes against a generative model).
...
- **Genetic — the gene.** The molecular replicator: a heritable recipe in nucleic acid.
...
- **Symbolic — the meme.** A unit of culture copied between agents. *Meme* here is the unit, the recipe; a meme-complex is the fauna built from memes, the superorganism sense used elsewhere in this doc. Unit and fauna are different levels, not competing definitions.
- **Fourth (open) — held as a program.** If a fourth line exists, its unit would be a generative recipe in the engineering sense — program, file, repository — kept outside the *-eme* family to mark it as the conjectural outsider. The framework does not claim such a line is forming.

Each unit sits in a four-level stack that is the same across the lines: a **pool** (the gene pool, the agent's learned repertoire, a culture, a repository), a **trace** that inscribes a unit (a chromosome, an engram, a text, a file), the **recipe** itself (gene, neme, meme, program), and the **phenotype** it produces (an organism, a behavior, an artifact, an output).
...
### Reviser as Goo-Ratchet, Effector as Darwin-Selected Outcome
...
- **Reviser is variation, the goo-ratchet.** The active work that keeps the line far from equilibrium.
...
### The Goo Test for a 4th Line

A new line requires a new modality of pattern transmission that can carry inheritance. Genetic, neuronal, and symbolic were each a new way of moving information across time. A 4th line, if it comes, would be identifiable by the same criterion: selection-mediated agent-to-agent inheritance, a new goo of pattern propagation. The framework does not claim such a line is forming.
...
The argument runs in five moves. First, the kinds of entropy and information available to name a goo. Second, Deacon's scheme, which the framework already cites. Third, Friston's decomposition of value, which explains what the neuronal line adds. Fourth, the patterns one might draw across the lines, and why most of them fail. Fifth, the structure that survives — the Goo Venn — in which each line plays one cybernetic function and natural selection sits at the center as the breakthrough.

### The kinds of entropy and information

**Boltzmann entropy.** **[established]** Boltzmann entropy is `S = k·ln W`. It counts the microstates `W` compatible with a macrostate. It is the thermodynamic measure of how dispersed energy and matter are. The Second Law says it tends to rise in an isolated system. Life is a dissipative structure: it holds its own internal entropy low by importing concentrated energy and exporting dispersed waste. This is the physics of *doing work*.

**Shannon entropy.** **[established]** Shannon entropy is `H = -Σ pᵢ·log pᵢ`. It measures the average uncertainty of a source over a set of messages. It is maximal when the messages are equally likely and falls as constraints make some messages more probable than others. A code, a grammar, or a convention reduces it by shrinking the space of legitimate messages. This is the physics of *representation*.

**Helmholtz free energy.** **[established]** Helmholtz free energy is `F = U − TS` — internal energy minus temperature times entropy. It is the energy available to do work at constant temperature, and systems fall toward its minimum. It matters here for two reasons. It is the namesake of the quantity Friston uses. And Helmholtz originated the idea of perception as unconscious inference, which is the root of the predictive brain.

**Friston free energy.** **[established as formalism; framework as a line-goo]** A system that persists must keep itself in a narrow set of expected states — it must minimize the long-run entropy of its own state distribution. The surprise that would measure this directly is intractable, so the system minimizes an upper bound on it: variational free energy, which takes the form *complexity minus accuracy*. It is lowered two ways: by perception, which updates the model, and by action, which changes the world to match the model. The result is an agent that resists dispersion into improbable states. Friston free energy is an information quantity (a divergence, Shannon-family) cast in the form of a thermodynamic potential (Helmholtz-family), which makes it the bridge term between Boltzmann and Shannon. This is the physics of *regulation by a model*.

**Kolmogorov complexity.** **[established as formalism; candidate only]** Kolmogorov complexity `K(x)` is the length of the shortest program that outputs `x` on a fixed universal machine. It measures algorithmic information — how incompressible an object is, how much it takes to *generate* it. It is high for random strings and low for regular ones, and it is uncomputable in general. It is the physics of *generation*, which is why it is a candidate for the fourth goo below. It is a candidate, not a commitment.

**Darwin.** **[established as a process, not an entropy]** Darwinian evolution by natural selection is not a measure of disorder. It is a process: variation, plus the selective retention of what works. It is the mechanism by which any of the substrates above becomes *heritable and cumulative across agents*. Its place in this scheme is argued below, and it is not one of the goos.
...
Two things in Deacon's picture matter for ours. The nesting runs from the most minimal (Shannon) to the most integrative (Darwin), and Darwin sits at the top as the term that confers usefulness. The Goo Venn keeps Shannon and Boltzmann as names and treats Darwin the way Deacon does — as the integrative term — but reorganizes the three from a *nesting* into a *Venn*, and adds Friston for the term Deacon did not have.
...
| Fourth (tbd) | Reviser | **open** | generation of variation — *Kolmogorov?* |
| — center — | Breakthrough | **Darwin** | natural selection; the synthesis that holds the four together |

**Why the mapping, function by function.** **[framework]** The Effector does work, and work is thermodynamic, so the Effector's substrate is Boltzmann entropy — and the genetic line is the line of raw dissipative replication. The Regulator maintains a system against perturbation, and the good-regulator theorem says every good regulator must contain a model of what it regulates, which is exactly Friston's free-energy minimizer — and the neuronal line is the line that regulates the body it rides on. The Modeler maintains a representation, and a representation is an encoding whose content is Shannon information — and the symbolic line is the line of codes, symbols, and shared representations. The Reviser generates and revises the references the regulator stabilizes against, which is the generation of novelty — and the fourth line, if it exists, would be the line that revises symbolic representations from outside the human substrate.
...
**The Reviser goo is open.** **[open]** The fourth function's substrate is the generation of variation, and the natural formalism for generation is algorithmic — Kolmogorov complexity, the length of the shortest program that produces a structure. A computational or digital fourth line that recombines and revises symbolic content would work on algorithmic information, which makes Kolmogorov the leading candidate. It is held loosely. Logical depth (the computational history behind a structure) and assembly-style measures are alternatives, though assembly measures are selection-flavored and would risk colliding with Darwin at the center. The fourth line's own identity is equally open; a digital or computational modality is the obvious guess, and the framework should not claim such a line is forming.


### Epistemic status and open questions

**Established.** The five formalisms (Boltzmann, Shannon, Helmholtz, Friston, Kolmogorov); the decomposition of expected free energy into pragmatic and epistemic value; the good-regulator theorem; Deacon's three nested conceptions.

**Framework interpretation, well supported.** The mapping of functions to goos (Effector–Boltzmann, Regulator–Friston, Modeler–Shannon); the Goo Venn and its derivation of the line order; the two-bit reflexivity square underlying that order; Darwin as the central breakthrough; the four-line cap; epistemic value arriving at the neuronal line; the heritable-unit naming (gene, neme, meme) and the four-level stack; the neme as the neuronal line's within-lifetime replicator, with attention as its selector.

**Open, held loosely.** The identity of the Reviser goo (Kolmogorov is the candidate). The identity of the fourth line. A possible flora/fauna corroboration (genetic and symbolic both yield flora, neuronal yields fauna), which should be checked against the flora-and-fauna account before it is leaned on.
...
The order in which cybernetic functions come online as the bramble grows (Regulator → Modeler → Reviser → Effector) matches the developmental arc of Joseph Campbell's hero's journey.

Act 1, the ordinary world: the hero begins inside an existing stable life. The Regulator is doing the work. Homeostasis is the starting condition.

Act 2A, initiation: a call to adventure pulls the hero out of the ordinary world. Mentors, allies, and enemies test her. She develops new models of the special world. The Modeler comes online.

Act 2B, transformation: facing great peril, the hero revises her goals. The ordeal forces meta-update of what she is pursuing. The Reviser comes online.

Act 3, return: the hero brings the elixir back to the ordinary world. The action she takes on her return propagates the transformation. The Effector executes the new pattern.

Evolution is not a sitcom where everything resets each episode. It is an epic adventure where everything that matters is transformed. The Effector is not the road back to the prior state. The Effector is what the prior state has become after the Regulator stabilized, the Modeler modeled, and the Reviser revised. The hero returns home, but the home she returns to is changed because she brought back the elixir, which IS the Effector in propagation-form, the new pattern going out into the world to alter what comes next.

Storytelling and evolutionary architecture both organize transformation along the same sequence: existing stability into new modeling into revised goals into transformed action. The handoff order picks up the same deep structure that storytelling traditions have intuited about how change unfolds in inheritance-bearing brambles.
...
- **Integration with Bennett's neural stack.** MET treats neural inheritance as one transition. Bennett's layered-modeling architecture gives the neuronal line its own internal structure with parallel rows. The combination is not in the literature.
...
*Epistemic status. This Part is a design specification for the framework's cognitive architecture. Its components are real paradigms (marked below); its synthesis — the single integrated system — is the framework's own, not a citation. The mapping from the architecture onto the framework's four functions is a framework commitment, HIGH confidence where it follows directly from the function definitions, flagged where it is an interpretive choice.*

## What Part II settles

Part II fixes what the Evolutionary Algorithm is, what it can and cannot do at each cell, and how much of it is built by hand versus how much grows on its own.
...
*Mapping status: HIGH. The two-mind split and the location of learning in the coupling follow directly from the Modeler, Reviser, and Adapter definitions in Part III. They are not new constructs.*

## Why there is only one mind at Eukaryote

At Eukaryote the unit's four active motivations are Persistence, Coordination, Interiority, and Self-Production (per the genetic Inheritance_Lines doc, Eukaryote row-slot). The Reviser motivation there is **Self-Production** — the autopoietic, work-producing function. It is not a built second cognitive system. The first Reviser that exists as built machinery is **Excitability**, the nervous tissue that arrives at Eumetazoa.
...
**The Evolutionary Algorithm is an evolution-as-learning process over a developmental, gene-regulatory-network-style genome — the deep-time mind — which grows and consolidates an active-inference online mind, in one self-transforming loop where the evolving *is* the learning.**
...
The components are real. Naming them is the teaching payoff: a reader who pulls any thread reaches real science.

- **Active inference** (Friston) — mature and prominent by 2015.
- **Deep reinforcement learning** — DQN landed in 2015.
...
What is **not** actual is the synthesis: a single loop in which a developmental-genome evolutionary process grows an active-inference online architecture *and* compresses that architecture's experience back into the genome, re-implementing lower layers beneath each new one.

The parts are named and real; the whole — the Evolutionary Algorithm / Evo-Algo — is the framework's original synthesis. No fabricated citation is attached to it. Nothing false is asserted about the world; a real set of parts is combined into an original, uncited whole.
...
- **Installed-but-dormant is visible, like eclipsed hardware.** The eclipsing-not-deletion pattern (Part I, 'Bramble Growth and the Relay') applies to software too: a dormant operation is groundwork the later cells will run on, not a feature bolted on later.
...
## The division of labor: macroevolution and the algorithm

A system that bootstrapped from slime-mold memory to language unsupervised would be open-ended evolution producing a general mind — a research dream, not a deliverable within the framework's scope. Complexity does not arrive on its own. It arrives in **twelve punctuated installs**, one per cell, when the next breakthrough is delivered. The labor splits the way biology splits it.

- **Macroevolution (the structural installs).** The structural innovations — the new function each cell brings. The breakthroughs are the installs; twelve cells are twelve macroevolutionary events.
- **The Evo-Algo plays microevolution, development, and learning.** Between punctuated innovations, it adapts whatever structure has been installed to the specific world, and — from Eumetazoa on — re-implements the lower layers beneath each new one.

The autonomous self-transformation is real but **bounded**: it fills in between installs. This is punctuated equilibrium, and macro versus micro evolution, as architectural principle.

*Mapping status: the identification of structural installs with macroevolution and the algorithm's fill-in with microevolution/development is a framework commitment, faithful to evolutionary biology.*

## The growth map across the trellis

What the deep-time mind must eventually enable on the online mind, cell by cell. The online mind's capability is gated by each cell's breakthrough (per the per-line Inheritance_Lines docs).
...
- **Vertebrate (Ch 6). Operant conditioning → model-free reinforcement learning.** Basal-ganglia actor-critic, reward prediction error updating the actor, procedural memory consolidating rewarded sequences. The breakthrough is Reinforcing.
...
- **Settlement-Human (Ch 10). Common Knowledge** — norms as stored regulation in the symbolic register.
...
This is also why the lineage's memory can fork at a line boundary and hand a new chassis "everything the previous Bramble had learned": the consolidated genome is precisely a transferable object.
...
Part III grounds the Venn architecture. The line-by-line diagram, the label libraries, and the row-slot tables appear in each of the Inheritance_Lines line docs (the shared framework is duplicated across all three; per-cell tables for a given line are in that line's file). The wider evolutionary story — personality as biographical signature, the cybernetic-inheritance literature, line-specific instantiation, bramble growth and the relay, the hero's-journey introduction order, the maturity gradient, and what ABHOP adds — sits in Part I above.

### **The bramble and the trellis**

The framework's central metaphor is a pair. *Bramble* is the unit — the cybernetic organism whose architecture and personality the framework specifies, instantiated as the developmental artifact at each of twelve cells. *Trellis* is the framework structure on which the bramble grows — the architectural scaffold of cybernetic functions, motivations, signatures, capacities, and breakthroughs, organized line by line and cell by cell across the twelve.
...
DeYoung's Stability and Plasticity metatraits do not sit at the triple-pair axes. They sit at single-primary positions in the Empire-Human Venn, where the Big Five empirically lives. Stability sits at the Effector (Commitment at Empire-Human). Plasticity sits at the Modeler (Representation at Empire-Human). See the symbolic Inheritance_Lines doc for the Empire-Human Big Five placements.
...
**Architecture A: Koolhaas-parallel.** PNS and HPG individual differences are negatively correlated across the population, parallel to the SNS-HPA structure on the Regulator × Reviser pair. The Effector × Modeler pair runs along the negative-slope axis as a ratio of fast-arm to slow-arm output. A second orthogonal axis is overall affiliation-system magnitude.
...
- **The Inheritance_Lines docs** (one per line: genetic, neuronal, symbolic) own the per-cell label libraries — the motivation names, capacity names, signature constructs, and breakthroughs at each cell. They are the authoritative source. When a name in this doc and a name in an Inheritance_Lines doc disagree, the Inheritance_Lines doc wins and this doc is the derivative to fix.
