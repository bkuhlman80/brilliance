...
- **After Ch 2 — What is Hidden.** The cell's interiority as a continuous act of refusal: the osmotic pull of the pond, the membrane as an argument the cell wins and wins again, the inside built — from its first afternoon — to keep the world from coming in. We arrive a long time later and discover we cannot reach into another person; the distance between us is not a flaw in the work, it is the work. *Source: the drafted poetic in `voice/written_Poetics.md` § "Chapter 2 — What is Hidden." The framework framing is prokaryotic osmotic regulation as the origin of interiority — adjacent to but not derived from cheryl_reflections's "Ashby's Theorems" section; substance lives in the drafted poetic itself.*
- **After Ch 3 — Symbiosis as a mode.** What it is for two lineages to commit to one unit. *Source: 'Symbiosis and Elaboration' (under Ecology Across Lines).*
- **After Ch 4 — One line seeding the next.** The last cell of a line gives rise to the first cell of the next; the genetic line ends by producing the nervous tissue the neuronal line is built on. *Source: 'The Bridge-Entity Pattern'.*
- **After Ch 5 — The relay (first retirement).** The first handoff has now happened in Bramble (genetic knob-1 retires at the line boundary); the deep-time analog is compressed here. *Source: 'Bramble Growth and the Relay'.*
- **After Ch 6 — Ashby's Theorems.** The two cybernetic theorems as one architectural claim: a surviving lineage is a regulator and therefore a distributed model of its environment (Good Regulator), and the model must be at least as varied as what it models (Requisite Variety). The substrate-independence that licenses the cross-line claim; the Cambrian arms race as illustration. *Source: 'Ashby's Theorems'.*
- **After Ch 7 — Attractors.** The two theorems as a dynamical attractor: a Regulator-bounded lineage is pulled toward building the Modeler or dies out, the "must" read as gravitational pull rather than tautology. The hero's-journey reading of why the Modeler comes online when it does. *Source: 'Attractors', with 'The Hero's Journey and the Introduction Order'.*
- **After Ch 8 — The maturity gradient.** What it is for motivations to consolidate from exploratory to infrastructural. First drafting traces. *Source: 'The Maturity Gradient'.*
- **After Ch 9 — Flora and fauna across lines.** Humans as flora to meme-fauna. Drafting traces increase. *Source: 'Flora and Fauna Across Lines'.*
- **After Ch 10 — Symbiosis vs elaboration at the symbolic line.** Cities forming as merger; bands forming as elaboration. Visible revision throughout. *Source: 'Symbiosis and Elaboration' and 'Elaboration at Scale'.*
- **After Ch 11 — Acceleration and pacing.** The saturation question gestured at without resolution. Chopped up. *Source: 'Acceleration and Pacing'.*
...
These commitments define what each line is and what it does: the inheritance modality it runs on, the kind of goo it ratchets, its variation-selection cycle, its pre-line groundwork, the machinery that instantiates the cybernetic dynamic, and the test for what would count as a new line. The cross-line cybernetic claim they rest on — surviving lineages as good regulators with internalized models — is grounded above and in `cybernetics_and_sentience doc`.
...
- **Neuronal** stores information in neural patterns, transmits via behavior and learning, modifies through experience. It ratchets affective goo (affective and motivational structure).
...
- **Neuronal line ratchets affective goo.** Sensorimotor reentrant signaling produces affective and motivational structure. Associative learning shapes this raw affective goo into the motivational architecture that coordinates behavior. These are the trait-level dispositional structures empirically documented from vertebrates and up.
...
*Epistemic status: the genetic line's Boltzmann-goo claim follows from Deacon's Shannon-Boltzmann-Darwin framework (2007a, 2007b, 2012). The neuronal line's affective-goo claim is supported by active inference (Friston) at the formal-mechanism level and by the behavioral-syndrome literature (Sih, Bell & Johnson 2004; Koolhaas) at the empirical-trait level. The symbolic claim is a framework-native application of the same principle.*
...
|--------|--------------------|---------------------|------------|
| Genetic | Self-Production (Boltzmann) | thermodynamic selection | Persistence |
| Neuronal | Excitability (FEP) | prediction-error selection | Boldness |
| Symbolic | Affiliation (Shannon) | exchange-based selection | Reciprocity |
...
### The Goo Test for a 5th Line

A new line requires a new modality of pattern transmission that can carry inheritance. Genetic, neuronal, and symbolic were each a new way of moving information across time. A 5th line, if it comes, would be identifiable by the same criterion: selection-mediated agent-to-agent inheritance, a new goo of pattern propagation. The framework does not claim such a line is forming.
...
### Row intervals by column

| Column | Row intervals (bottom to top) |
|--------|-------------------------------|
| Genetic | ~500 MY, ~1.5 BY, ~1.2 BY (variable) |
| Neuronal | ~130 MY, ~220 MY, ~134 MY (flat, ~161 MY mean) |
| Symbolic | ~90 KY, ~5 KY, ~2 KY (accelerating) |

The genetic and neuronal lines do not accelerate within themselves. Only the symbolic line accelerates, and the mechanism is specific. Symbols mean by network position, which enables unbounded reflexivity. Each generation can take the previous generation's self-understanding as an object and operate on it, with no architectural ceiling. Genes cannot be about genes. Neurons model lower neural layers but cannot operate on the modeling-relation itself. Symbols can take their own operations as objects, indefinitely. The symbolic line accelerates because its goo removes the architectural limit on reflexive iteration that constrains both prior lines.

### Generation rates across columns

The slow loop (carrier instantiation, selection, and deprecation) runs at vastly different rates across columns. Genetic units turn over across millions of years. Symbolic-level institutional carriers turn over across decades. Same dynamic, different rates.

### Caveats

A log-linear trend fits the symbolic column's four row-emergence dates reasonably well, but the inter-stage ratios bounce between roughly 2.5× and 18×. The acceleration is real. The smoothness is not. Perreault (2012) established that rates of cultural evolution are inversely correlated with measurement interval, similar to biological rates. Some apparent acceleration in the symbolic column reflects stage-resolution. Deep history is chunked into fewer rows than recent history. The phase change at band-emergence is real. The within-line smooth acceleration is partly real and partly a chunking artifact.

### Column emergence timing

New columns consistently emerge well before the parent column would have produced its next row.

- Neuronal founding: Bilaterians emerge ~250 MY after Eumetazoa. Genetic row intervals run from ~500 MY to ~1.5 BY, so the neuronal column founds well before the genetic column would have produced a next row.
- Symbolic founding: Bands emerge ~66 MY after Primates, about 41% into the ~161 MY mean neuronal row interval.

Sample size is 2. The qualitative pattern holds across both transitions: a new column emerges well before the parent column reaches its next row. A single percentage does not summarize the pattern, because the genetic row intervals are variable rather than uniform. The pattern is suggestive but not architecturally established.

### Saturation

The symbolic line has a floor. Stage lifespan cannot compress below the cycle-times of the cycles that maintain its goo. Three versions of this claim, ordered by defensibility:

- **Strong:** stages compress until they fall below line-renewal cycles, breaking the inheritance loop. Probably false. Cycles likely rescale rather than break.
- **Moderate:** stages compress until they fall below the human developmental cycle (~20 years), at which point symbolic transmission requires bypassing biological humans. Defensible but exposed.
- **Weak (recommended):** the saturation question is the column-emergence question. When stage compression approaches biological floors, the line exits through new-column emergence rather than continuing to add rows. Consistent with the cross-column emergence pattern, and it does not require the inheritance loop to break.
